<template>
	<g>
		...
		<g v-else>
			...
			<line v-for="n in xSteps" :x1="yAxisSpace+padding+(n*innerWidth/xSteps)" :y1="padding" :x2="yAxisSpace+padding+(n*innerWidth/xSteps)" :y2="innerHeight+padding" :style="'stroke: '+color+'; opacity: 0.35;'"/>
		</g>
	...
	computed: {
		...
		computedLabel () {
			return this.labels.map((l, index) => {
				...
				const y = this.yStepSize ? this.innerHeight + this.padding + (this.needRotateLabel ? 10 : this.xAxisSpace) : (index + 0.5) * this.innerHeight / this.labels.length + this.fontSize / 2 + this.padding
